...
hermiteSpl *pchip_set(int n, 
  ...
  if((H = (hermiteSpl *)ALLOC(1,sizeof(hermiteSpl))) == NULL) exit(EXIT_FAILURE);

  // fill-in the struct -- not sure of malloc

  H->npts = n;

  if((H->x = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((H->a = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((H->b = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((H->c = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((H->d = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  
  if((h = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((delta = (double *)ALLOC(n,sizeof(double))) == NULL) exit(EXIT_FAILURE);
...
void  pchip_val(hermiteSpl *H, double xval, int extrapol,
		double *yval, double *yvald, double *yvaldd) {
  ...
  if(extrapol == 0 && (i == 0 || i == H->npts)) {
    ...
    *yvald = H->b[i];
    *yvaldd = 0; // This is the case where ydd does not exists;
  ...
  *yvald = H->b[i] + s * (2 * H->c[i] + 
			  s * (3 * H-> d[i]));  

  *yvaldd = 2 * H->c[i] + s * 6 * H->d[i]; 
