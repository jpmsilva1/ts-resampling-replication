...
phi_bumps *bumps_set(hermiteSpl *H, double *loss_args) {
  ...
  if((critical_idx = (int *)ALLOC(nB,sizeof(int))) == NULL) exit(EXIT_FAILURE);
  if((B = (phi_bumps *)ALLOC(1,sizeof(phi_bumps))) == NULL) exit(EXIT_FAILURE);

  B->n = 0;
  if((B->bleft = (double *)ALLOC(nB,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((B->bmax = (double *)ALLOC(nB,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  if((B->bloss = (double *)ALLOC(nB,sizeof(double))) == NULL) exit(EXIT_FAILURE);
  ...
  while(i < j - 1) {
	...
	if(isfinite(B->bmax[B->n]) && isfinite(B->bleft[B->n])) {
	...
	if(isfinite(B->bmax[B->n-1]) && isfinite(B->bleft[B->n])) {
  ...
  if(B->n > 0) { // still updates to be done
    ...
    if(!isfinite(B->bmax[0])) B->bloss[0] = B->bloss[1];
    if(!isfinite(B->bmax[B->n])) B->bloss[B->n] = B->bloss[B->n-1];
