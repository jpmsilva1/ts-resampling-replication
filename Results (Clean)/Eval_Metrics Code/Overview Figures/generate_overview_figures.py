#!/usr/bin/env python3
"""Generates Figures 7 and 8 from the paper "Resampling strategies for imbalanced
time series forecasting" (Moniz, Branco & Torgo, 2017).

Figure 7: F1 scores by dataset, ordered naturally (DS01–DS20).
Figure 8: F1 scores by dataset, ordered by ascending number of rare cases.

Each figure is a 4×5 grid (4 rows: None/UNDER/OVER/SmoteR, 5 columns:
lm/svm/mars/rf/rpart). Each subplot shows multiple lines for different
workflows, with a shared legend.

Source data:
  Results (Clean)/Results Data/raw_iterations_by_dataset_v2/*.csv (data_v2 run --
  same run RMSE/SERA use; see CLAUDE.md "2026-08-28 -- F1 re-extracted from data_v2")

Output:
  Results (Clean)/Evaluation Metrics/Figures/Overview/fig7_f1_by_dataset.png
  Results (Clean)/Evaluation Metrics/Figures/Overview/fig8_f1_by_rare_cases.png
"""
import sys
...
# Model families, in column order
FAMILIES = ["lm", "svm", "mars", "rf", "rpart"]

# Resampling groups, in row order
GROUPS = ["None", "UNDER", "OVER", "SmoteR"]

# The order of datasets by ascending number of rare cases (from task description)
RARE_ORDER = [
...
def _parse_workflow(wf: str):
    ...
    # Resampled: mc.<family>_<GROUP><BIAS>
    strategy = parts[1]

    # Extract group and bias
    if strategy.startswith("UNDER"):
        group = "UNDER"
        bias = strategy[5:]  # "B", "T", or "TPhi"
...
def load_f1_data() -> pd.DataFrame:
    """Load all F1 data from raw CSVs."""
    ...
    # Average F1 over iterations, grouped by (dataset_id, workflow)
    agg = df.groupby(["dataset_id", "workflow", "model_family", "group", "bias"])["F1"].mean().reset_index()
    agg = agg.rename(columns={"F1": "f1_mean"})
...
def plot_figures(data: pd.DataFrame):
    ...
    # Figure 7: natural order
    _plot_figure(data, NATURAL_ORDER,
                 OUT_DIR / "fig7_f1_by_dataset.png",
                 "F1 by Dataset (Natural Order)")

    # Figure 8: rare-case order
    _plot_figure(data, RARE_ORDER,
                 OUT_DIR / "fig8_f1_by_rare_cases.png",
                 "F1 by Dataset (Sorted by Rare Cases)")

    # Figure 9: imbalance-ratio order
    _plot_figure(data, IMBALANCE_ORDER,
                 OUT_DIR / "fig9_f1_by_imbalance_ratio.png",
                 "F1 by Dataset (Sorted by Imbalance Ratio)")

    # Figure 10: %Rare order
    _plot_figure(data, PCT_RARE_ORDER,
                 OUT_DIR / "fig10_f1_by_pct_rare.png",
                 "F1 by Dataset (Sorted by %Rare)")


def _plot_figure(data: pd.DataFrame, dataset_order: list, out_path: Path, title: str):
    """Plot a single figure with given dataset order. Sized for its own
    full-width landscape page (not stacked with its SERA counterpart), which
    is what actually buys legibility for a dense 4x5 grid -- font size alone
    can't compensate for being squeezed into a shared portrait page."""
    fig, axes = plt.subplots(4, 5, figsize=(22, 14))
    fig.suptitle(title, fontsize=28, fontweight='bold')

    # Create a mapping of dataset_id to position (1-20)
    dataset_to_pos = {ds: i+1 for i, ds in enumerate(dataset_order)}

    # Prepare line styles and colors for legend
    line_styles = {
        "Original": ("solid", C_DARK),
        "B": ("dashed", C_TEAL),
        "T": ("dotted", C_TEAL),
        "TPhi": ("dashdot", C_TEAL),
        "arima": ("dotted", C_GOLD),
        "bdes": ("dashdot", C_CORAL),
    }
    ...
    for row_idx, group in enumerate(GROUPS):
        for col_idx, family in enumerate(FAMILIES):
            ...
            if group == "None":
                # "None" row: only Original, ARIMA, BDES
                for bias in ["Original", "arima", "bdes"]:
                    if bias == "Original":
                    ...
                    subset = data[mask]
                    if len(subset) > 0:
                        # Map to positions, then sort by position -- subset keeps
                        # dataset_id's groupby order, which isn't dataset_order.
                        ordered = subset.assign(pos=subset["dataset_id"].map(dataset_to_pos)).sort_values("pos")
                        lines_to_plot[bias] = (ordered["pos"], ordered["f1_mean"])
            else:
                # Other rows: Original, B, T, TPhi, ARIMA, BDES for this family
                for bias in ["Original", "B", "T", "TPhi", "arima", "bdes"]:
                    if bias == "Original":
            ...
            # Row labels on right
            if col_idx == 4:
                ax.text(1.14, 0.5, group, transform=ax.transAxes,
    ...
    fig.legend(handles=legend_elements, loc="lower center", ncol=6,
              bbox_to_anchor=(0.5, -0.02), fontsize=26, frameon=False,
              handlelength=3.5, handletextpad=0.8, columnspacing=2.2)
